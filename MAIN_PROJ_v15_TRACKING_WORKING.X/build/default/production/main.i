...
# 1 "./dc_motor_struct.h" 1
# 55 "./dc_motor_struct.h"
struct DC_motor {
    char power;
    char direction;
...
volatile unsigned char G_rfid_buf[16];
volatile unsigned int G_time_ms;

volatile unsigned char G_rfid_retrieved;
volatile unsigned char G_run;




void __attribute__((picinterrupt("high_priority"))) hp_rfid(void) {



    if (PIR1bits.RCIF) {
        get_packet_serial(G_rfid_buf);
        G_rfid_retrieved = 1;
    }
...
void __attribute__((picinterrupt("low_priority"))) lp_timer(void) {
...
void main(void) {
    ...
    cont.gain = 40;
    cont.time_to_turn = 0;
    cont.ir_threshold = 350;
    cont.ir_min = 47000;
    cont.ir_buf[6];
    cont.i = 0;
    cont.finding_direction = 0;
    cont.steer_action[220];


    unsigned char chksm[5];
    unsigned char j = 0;
...
# 406 "main.c"
        if ((G_rfid_retrieved == 1) & (G_run == 1)) {
            ...
            chksm[0] = G_rfid_buf[0] | G_rfid_buf[1];
            chksm[1] = G_rfid_buf[2] | G_rfid_buf[3];
            chksm[2] = G_rfid_buf[4] | G_rfid_buf[5];
            chksm[3] = G_rfid_buf[6] | G_rfid_buf[7];
            chksm[4] = G_rfid_buf[8] | G_rfid_buf[9];
