...
void delay_s(char seconds) {
    ...
    unsigned int cycles = 20 * seconds;
    unsigned int i = 0;
    for (i = 0; i < cycles; i++) {
...
void init_ir(void) {
    ...
    TRISAbits.RA3 = 1;
    TRISAbits.RA4 = 1;
    CAP2CONbits.CAP2M = 0b0110;
    CAP3CONbits.CAP3M = 0b0110;
...
unsigned int ir_filter(unsigned char sensor) {
    ...
    unsigned char same_values_flag = 1;
    while (i < 4) {
        array[i] = get_ir(sensor);
